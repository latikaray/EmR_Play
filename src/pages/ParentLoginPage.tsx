...
const ParentLoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await signIn(formData.email, formData.password, 'parent');
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-muted-foreground font-comic">
            Welcome back! Track your child's emotional growth 🌟
          </p>
        </div>
        ...
        <Card className="shadow-fun bg-card/80 backdrop-blur border-2 border-primary/20 hover-lift">
          <CardHeader className="text-center">
            ...
            <CardDescription className="font-comic">
              Access your parenting dashboard
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-gradient-primary text-primary-foreground shadow-fun">
          <CardContent className="p-4 text-center">
            <h4 className="font-bold font-comic mb-2">🔒 Secure Parent Portal</h4>
            <p className="text-sm opacity-90 font-comic">
              Your parenting data and child insights are protected with enterprise-grade security.
            </p>
          </CardContent>
        </Card>
      </div>
